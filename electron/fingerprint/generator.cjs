...
class FingerprintGenerator {
  ...
  generateUserAgent(os, chromeVersion) {
    const webkit = '537.36';
  ...
  generateScreen(os) {
    ...
    if (os === 'macOS') {
      devicePixelRatio = this.randomChoice([2, 2.5, 3]);
    } else {
    ...
    return {
      ...
      availHeight: resolution.height - (os === 'Windows' ? 40 : 25),
      colorDepth,
      pixelDepth,
      devicePixelRatio
    };
  ...
  generateHardware() {
    return {
      hardwareConcurrency: this.randomChoice([4, 6, 8, 12, 16]),
